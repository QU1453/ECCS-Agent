...
**跨境电商智能体工作台（日本市场）** — 面向跨境卖家的 AI 工作台：选品验证、供应商对比、Listing 创建与上架，同时保留买家客服能力（订单 / 物流 / 售后 / 推荐）。
...
跨境电商场景下，卖家在素人阶段最高频的两件事是 **选品（找供应商）** 与 **写 Listing（上架）**，海外（日本）消费者也持续存在订单、物流、退换货等咨询。本项目构建一套 **多智能体工作台**：

- **卖家工作台（当前主线）**：选品四步验证（需求 / 竞争 / 利润）+ 供应商搜索对比；Listing 起草、图片合规自检、定价与 FBA/FBM 建议；
- **买家客服（保留能力）**：订单物流查询、退换货办理、商品推荐，中日双语一键切换；
